...
export const endpoints = {
  ...
  icon: (icon: string) => `http://openweathermap.org/img/wn/${icon}.png`,
  weekWeather: (lat: number, lon: number) =>  `https://api.openweathermap.org/data/3.0/onecall?lat=${lat}&lon=${lon}&exclude=minutely,hourly,current,alerts&appid=${API_KEY}&units=metric`,
}